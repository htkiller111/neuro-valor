...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		...
		List<CandleStick> allSticks = Tools.loadMSTToSubsequentValues(new File("data/KREZUS.mst"));
		final List<CandleStick> sticks = new ArrayList<CandleStick>(allSticks.subList(allSticks.size()-2000, allSticks.size()));
		...
		BasicNetworkEvaluator evaluator = new BasicNetworkEvaluator() {
			...
			public double evalute(BasicNetworkStructure bnn) {
				...
				BasicNetwork nn = EncogUtility.simpleFeedForward(CandleStick.FACTORS*bnn.getInputCount(), bnn.getFirstCount(), bnn.getSecondCount(), 1, false);
				
				int trainingPoints = sticks.size()-1-waiting-tests-bnn.getInputCount();
				double[][] in = new double[trainingPoints][];
				double[][] out = new double[in.length][];
				for (int i=0; i<in.length; ++i) {
					double[] ini = new double[CandleStick.FACTORS*bnn.getInputCount()];
					...
					double neededLevel = 1.01*sticks.get(i+bnn.getInputCount()-1).getCloseValue();
					boolean returned = false;
					for (int j=0; j<waiting; ++j) {
						if (sticks.get(i+bnn.getInputCount()+j).getCloseValue() > neededLevel) {
							returned = true;
							break;
						}
					}
				...
				for (int i=trainingPoints; i<sticks.size()-bnn.getInputCount()-waiting-1; ++i) {
					double[] testIn = new double[CandleStick.FACTORS*bnn.getInputCount()];
					for (int j=0; j<bnn.getInputCount(); ++j) {
					...
					boolean returned = false;
					for (int j=0; j<waiting; ++j) {
						if (sticks.get(i+bnn.getInputCount()+waiting).getCloseValue() > neededLevel) {
							returned = true;
							break;
						}
					}
		...
		RemoteComputationDispatcherConnection rc = new RemoteComputationDispatcherConnection("wierzba.ki.agh.edu.pl", 44444);
		List<BasicNetworkStructure> population = new ArrayList<BasicNetworkStructure>();
		for (int i=0; i<NEURONNET_POPULATION_SIZE; ++i) {
			population.add(new BasicNetworkStructure(evaluator, randomize(100, 0.8), CandleStick.FACTORS*randomize(100, 0.8), CandleStick.FACTORS*randomize(100, 0.8), false));
		}
